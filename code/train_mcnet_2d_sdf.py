...
from utils import losses, metrics, ramps, val_2d ,losses_sdf
...
parser.add_argument('--exp', type=str, default='MCNet_sdf', help='experiment_name')
parser.add_argument('--model', type=str, default='mcnet2d_v3', help='model_name')
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            for idx in range(num_outputs):
                ...
                with torch.no_grad():
                    gt_dis =losses_sdf.compute_sdf(label_batch[:labeled_bs][:].long().cpu().numpy(),  y[:labeled_bs, 0, ...].shape)
                    gt_dis = torch.from_numpy(gt_dis).float().cuda()
                loss_seg_sdf += consistency_criterion(y[:labeled_bs, 0, ...], gt_dis) # also do this with tanh output
            ...
            loss = args.lamda * (loss_seg_dice + 0.3 * loss_seg_sdf) + consistency_weight * loss_consist
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss : %03f, loss_d: %03f, loss_cosist: %03f, loss_sdf: %03f' % (iter_num, loss, loss_seg_dice, loss_consist,loss_seg_sdf))
            if iter_num % 20 == 0:
